...
bool rulename2typename(const SqlAstNode *item, std::string& type_name)
{
  ...
  for (std::map<sql::symbol, std::string>::const_iterator i= subst_rules.begin(), i_end= subst_rules.end(); i != i_end; ++i)
  {
...
{
  return CS_COLLATION_SETTER(db_Schema, obj.content(), defaultCharacterSetName, defaultCollationName,
    db_Catalog, container.content(), defaultCharacterSetName, defaultCollationName, explicit_cs);
}


Cs_collation_setter cs_collation_setter(db_mysql_TableRef obj, db_SchemaRef container, bool explicit_cs)
{
  return CS_COLLATION_SETTER(db_mysql_Table, obj.content(), defaultCharacterSetName, defaultCollationName,
    db_Schema, container.content(), defaultCharacterSetName, defaultCollationName, explicit_cs);
}


Cs_collation_setter cs_collation_setter(db_ColumnRef obj, db_mysql_TableRef container, bool explicit_cs)
{
  return CS_COLLATION_SETTER(db_Column, obj.content(), characterSetName, collationName,
    db_mysql_Table, container.content(), defaultCharacterSetName, defaultCollationName, explicit_cs);
}
