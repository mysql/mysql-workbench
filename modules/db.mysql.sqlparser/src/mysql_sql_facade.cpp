...
bool MysqlSqlFacadeImpl::parseSelectStatementForEdit(const std::string &sql, std::string &schema_name, std::string &table_name, String_tuple_list &column_names)
{
  bool ret_val = false;
...
std::string MysqlSqlFacadeImpl::getTypeDescription(grt::BaseListRef type_node, std::vector<std::string> *additional_type_data_paths)
{
...
bool MysqlSqlFacadeImpl::parseRoutineDetails(const std::string &sql, std::string &type, std::string &name, String_tuple_list &parameters, std::string& return_value, std::string& comments)
{
  bool ret_val = false;
...
bool MysqlSqlFacadeImpl::parseDropStatement(const std::string &sql, std::string &object_type, std::vector<std::pair<std::string, std::string> > &object_names)
{
  bool ret_val = false;
...
grt::DictRef MysqlSqlFacadeImpl::parseStatement(const std::string &sql_statement, int server_version, int utf, const std::string &sql_mode)
{
  grt::DictRef ret_val = grt::DictRef();
  std::set<std::string> charsets(_charsets.begin(), _charsets.end());
...
grt::StringRef MysqlSqlFacadeImpl::concatenateTokens(MySQLRecognizerTreeWalker &walker, std::unordered_set<int> &stop_symbols, const std::string &separator)
{
  ...
  return grt::StringRef(data);
}


grt::StringListRef MysqlSqlFacadeImpl::createList(MySQLRecognizerTreeWalker &walker, int separator_symbol, std::unordered_set<int> &stop_symbols)
{
  grt::StringListRef list = grt::StringListRef(get_grt());

  std::unordered_set<int> item_separators(stop_symbols.begin(), stop_symbols.end());
  item_separators.insert(separator_symbol);

  // Dummy move to simplify logic inside the iteration below
  walker.previous();

  do 
  {
    walker.next();
    list.insert(concatenateTokens(walker, item_separators));
  }while (stop_symbols.find(walker.token_type()) == stop_symbols.end());
...
/*
 * parseUserDefinition: Provides a dctionary with the definition of a user
 * assumption : the walker is at the starting point of a user definition
 * returned dictionary:
 ...
 */
grt::DictRef MysqlSqlFacadeImpl::parseUserDefinition(MySQLRecognizerTreeWalker &walker)
{
  // Gets the user
  std::string user_str = walker.token_text();
  grt::DictRef user_dict = grt::DictRef(get_grt());

  user_dict.set("user", grt::StringRef(user_str));

  if (walker.token_type() == CURRENT_USER_SYMBOL)
  {
    // Skips the current user and parenthesis
    walker.next();
    walker.next();
    walker.next();
  }
  ...
  {
    // The user token(s) has been read, skips to the next token
    ...
    if (walker.token_type() == AT_SIGN_SYMBOL)
    {
      walker.next();
      user_dict.set("host", grt::StringRef(walker.token_text()));

      // The host token(s) have been read, sekips to the next token
      if (!walker.next_sibling())
      {
        walker.up();
        walker.next_sibling();
  ...
  if (walker.token_type() == IDENTIFIED_SYMBOL)
  {
    walker.next();
    if (walker.token_type() == BY_SYMBOL)
    {
      walker.next();

      if (walker.token_type() == PASSWORD_SYMBOL)
        walker.next();

      user_dict.set("id_method", grt::StringRef("PASSWORD"));
      user_dict.set("id_string", grt::StringRef(walker.token_text()));
    ...
    else if (walker.token_type() == WITH_SYMBOL)
    {
      walker.next();
      user_dict.set("id_method", grt::StringRef(walker.token_text()));
      walker.next();

      if (walker.token_type() == AS_SYMBOL)
      {
        walker.next();
        user_dict.set("id_string", grt::StringRef(walker.token_text()));
        walker.next();
...
grt::DictRef MysqlSqlFacadeImpl::parseGrantStatement(MySQLRecognizer &recognizer)
{
  grt::DictRef data = grt::DictRef(get_grt());
  std::unordered_set<int>terminators;
  terminators.insert(ANTLR3_TOKEN_EOF);
  ...
  data.set("privileges", createList(walker, COMMA_SYMBOL, terminators));

  // Skips the PRIVILEGE_TARGET token
  walker.next();

  // Skips the ON token
  walker.next();

  // Doesn't hurt to let the others so we are ok to go
  terminators.insert(TO_SYMBOL);

  // Sets the privilege target as a single text string
  data.set("target", concatenateTokens(walker, terminators, ""));

  //Skips the TO symbol
  walker.next();

  // Now Parses the user definitions
  terminators.clear();
  terminators.insert(ANTLR3_TOKEN_EOF);
  terminators.insert(WITH_SYMBOL);
  terminators.insert(REQUIRE_SYMBOL);


  grt::DictRef users = grt::DictRef(get_grt());
  data.set("users", users);

  while (terminators.find(walker.token_type()) == terminators.end())
  {
    grt::DictRef user_definition = parseUserDefinition(walker);
    
    std::string user_str = grt::StringRef::cast_from(user_definition.get("user"));

    users.set(user_str, user_definition);
  ...
  // Skips the REQUIRE info from the GRANT statement
  bool options = walker.token_type() == WITH_SYMBOL;
  if (!options)
      options = walker.advance_to_type(WITH_SYMBOL, false);

  if (options)
  {
    walker.next();
    if (walker.token_type() == GRANT_SYMBOL)
    {
      data.set("option", grt::StringRef("GRANT"));
    }
    else
    {
      data.set("option", grt::StringRef(walker.token_text()));
      walker.next();
      data.set("option_value", grt::StringRef(walker.token_text()));
    }
  }
