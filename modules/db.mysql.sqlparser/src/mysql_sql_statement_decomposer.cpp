...
{
  NULL_STATE_KEEPER
    int err_count= process_sql_statement(sql, select_statement, boost::bind(
    &Mysql_sql_statement_decomposer::do_decompose_query, this, _1));
...
{
  NULL_STATE_KEEPER
  int err_count= process_sql_statement(sql, select_statement, boost::bind(&Mysql_sql_statement_decomposer::do_decompose_view, this, _1));
  return (err_count ? 0 : 1);
}


int Mysql_sql_statement_decomposer::process_sql_statement(const std::string &sql, SelectStatement::Ref select_statement, ProcessSqlStatement do_process_sql_statement_cb)
{
...
int Mysql_sql_statement_decomposer::process_sql_statement(const std::string &sql, SelectStatement::Ref select_statement, Mysql_sql_parser_fe &sql_parser_fe)
{
...
Mysql_sql_parser_base::Parse_result Mysql_sql_statement_decomposer::decompose_query(const SqlAstNode *select_init)
{
  ...
  // from clause
  {
    ...
    else if (const SqlAstNode *derived_table_list= select_part2->subitem(sql::_select_into, sql::_select_from, sql::_join_table_list, sql::_derived_table_list))
    {
      ...
      BOOST_FOREACH (const SqlAstNode *table_factor, table_factor_list)
      {
        ...
        // subquery
        {
          ...
          if (from_item.subquery.empty())
          {
            if (const SqlAstNode *parenthesis_open= table_factor->subitem(sql::_40)) // sql::_40 == ASCII('('); sql::_41 == ASCII(')')
            {
              from_item.subquery= table_factor->restore_sql_text(_sql_statement, parenthesis_open, table_factor->subitem(sql::_41));
      ...
      BOOST_FOREACH (const SqlAstNode *select_item, *select_item_list->subitems())
      {
        if (select_item->name_equals(sql::_select_item))
        {
          ...
          // expression
          {
            ...
            {
              const SqlAstNode *simple_expr= expr->subitem(sql::_bool_pri, sql::_predicate, sql::_bit_expr, sql::_simple_expr);
              while (simple_expr)
              {
                ...
                if (simple_expr->find_subseq(sql::_40, sql::_expr, sql::_41)) // sql::_40 == ASCII('('); sql::_41 == ASCII(')')
                  simple_expr= simple_expr->subitem(sql::_expr, sql::_bool_pri, sql::_predicate, sql::_bit_expr, sql::_simple_expr);
                else
                  simple_expr= NULL;
              }
            }
...
void Mysql_sql_statement_decomposer::expand_wildcards(SelectStatement::Ref select_statement, db_SchemaRef &db_schema, grt::ListRef<db_Schema> &db_schemata)
{
      ...
      while (context_statement)
      {
        BOOST_FOREACH (FromItem &from_item, context_statement->from_items)
        {
          ...
          if (context_table.empty() || alias == context_table)
          {
            ...
            if ((context_schema.empty() && schema.empty())
                ...
                || (context_schema == active_schema && schema.empty()))
            {
              ...
              {
                std::string affective_schema= !schema.empty() ? schema : (!context_schema.empty() ? context_schema : active_schema);
                db_SchemaRef db_schema= find_named_object_in_list(db_schemata, affective_schema, _case_sensitive_identifiers);
                db_TableRef db_table= find_named_object_in_list(db_schema->tables(), table, _case_sensitive_identifiers);
                ...
                {
                  db_ViewRef db_view= find_named_object_in_list(db_schema->views(), table, _case_sensitive_identifiers);
                  if (db_view.is_valid())
                  {
                    for(grt::StringListRef::const_iterator col = db_view->columns().begin(); col != db_view->columns().end(); ++col)
                    {
                      SelectItem new_select_item;
                      new_select_item.schema= schema;
                      new_select_item.table= from_item.alias;
