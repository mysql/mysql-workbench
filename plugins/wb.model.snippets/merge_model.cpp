...
struct tolower_pred
{
	std::string tolower(const std::string& arg){return base::tolower(arg);};
};
...
{
  grt::BaseListRef args(obj->get_grt());
  grt::Module *module= obj->get_grt()->get_module("Workbench");
...
void copy_additional_data(db_TableRef obj,std::string old_name,TOwner new_owner)
{
  ...
  bec::GRTManager *grtm = bec::GRTManager::get_instance_for(obj->get_grt());
  grt::BaseListRef args(obj->get_grt());
  grt::Module *module= obj->get_grt()->get_module("Workbench");
...
void merge_schema(//workbench_DocumentRef document,
                  const db_SchemaRef& target_schema,
...
void merge_catalog (grt::Module *module, db_CatalogRef& dest_cat,const db_CatalogRef src_cat)
{
  ...
  {
    if(dest_cat->schemata().count() == 1)
    {//Merge to exiting schema regardless of name
...
void merge_diagrams (grt::ListRef<workbench_physical_Diagram>& dest_diagrams, const grt::ListRef<workbench_physical_Diagram>& src_diagrams, const GrtObjectRef& dst_owner)
{
