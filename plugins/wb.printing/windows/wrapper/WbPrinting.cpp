...
{
  return wbprint::getPageCount(model_DiagramRef::cast_from(grt::BaseListRef::cast_from(view->get_unmanaged_object()).get(0)));
}
...
{
  return wbprint::printPageHDC(model_DiagramRef::cast_from(grt::BaseListRef::cast_from(view->get_unmanaged_object()).get(0)),
    page, (HDC)hdc->ToPointer(), width, height);
}
...
{
  app_PageSettingsRef settings = wbprint::getPageSettings(model_DiagramRef::cast_from(grt::BaseListRef::cast_from(value->get_unmanaged_object()).get(0)));
