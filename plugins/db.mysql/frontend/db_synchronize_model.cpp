...
public:
  ModelSchemaMatchingPage(grtui::WizardForm *form, DbMySQLSync *be, const char *name= "selectSchemata",
                          ...
                          const std::string &right_name = "Database")
  ...
  virtual void enter(bool advancing)
  {
    if (advancing)
    {
      ...
      if (_db_be && _db_be->db_conn())
        values().set("server_is_case_sensitive", grt::IntegerRef((int)_db_be->db_conn()->get_dbc_connection()->getMetaData()->storesMixedCaseIdentifiers()));
      else
        values().set("server_is_case_sensitive", grt::IntegerRef(1)); // By default set it to true, so we're on the safe side.
