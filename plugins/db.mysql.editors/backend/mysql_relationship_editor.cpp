...
{
  return strfmt("'%s'  (%s)  '%s'", get_left_table_name().c_str(), get_caption().c_str(), get_right_table_name().c_str());
}
...
{
  return strfmt("'%s' (%s) '%s'", get_right_table_name().c_str(), get_extra_caption().c_str(), get_left_table_name().c_str());
}
...
std::string RelationshipEditorBE::get_left_table_info()
{
  ...
  if (fk.is_valid())
    ...
    {
      text.append(strfmt("%s: %s%s\n", fk->columns()[i]->name().c_str(), 
                         fk->columns()[i]->formattedRawType().c_str(),
