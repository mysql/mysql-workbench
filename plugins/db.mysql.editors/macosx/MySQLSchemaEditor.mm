...
static void call_refresh(void *theEditor) {
    ...
    [editor performSelectorOnMainThread:@selector(refresh)
                             withObject:nil
                          waitUntilDone:YES];
}
...
- (instancetype)initWithModule:(grt::Module *)module
    ...
    self = [super initWithNibName:@"MySQLSchemaEditor"
                           bundle:[NSBundle bundleForClass:[self class]]];
    if (self != nil) {
        // load GUI. Top level view in the nib is the NSTabView that will be docked
        // to the main window
        [self loadView];
        ...
        // fill the charset and collation Popups with the list of supported
        // charsets/collations taken from the backend
        ...
        [charsetPopup
         insertItemWithTitle:[NSString
                              stringWithUTF8String:DEFAULT_COLLATION_CAPTION]
         atIndex:0];
        MFillPopupButtonWithStrings(
                                    collationPopup,
                                    mBackEnd->get_charset_collation_list(DEFAULT_CHARSET_CAPTION));
        ...
        // whenever the backend thinks its needed to refresh the UI from the backend
        // data (ie, the
        // edited object was changed from somewhere else in the application)
        mBackEnd->set_refresh_ui_slot(
                                      std::bind(call_refresh, (__bridge void *)self));
        ...
        if (mBackEnd->is_editing_live_object()) {
            ...
            [[[[commentText enclosingScrollView] superview] viewWithTag:103]
             setHidden:NO];
            [mRefactorButton setEnabled:NO];
        }
...
- (void)refresh {
    if (mBackEnd) {
        ...
        if (!mChanging)
            [commentText
             setString:[NSString stringWithCPPString:mBackEnd->get_comment()]];
...
- (void)updateCollationPopup:(const char *)charset {
    MFillPopupButtonWithStrings(collationPopup,
                                mBackEnd->get_charset_collation_list(charset));
    [[collationPopup menu] insertItem:[NSMenuItem separatorItem] atIndex:0];
    [collationPopup
     insertItemWithTitle:[NSString
                          stringWithUTF8String:DEFAULT_COLLATION_CAPTION]
