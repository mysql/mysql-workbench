...
public:
  ...
  inline void translate(const base::Point &p) { cairo_translate(cr, p.x, p.y); }
  inline void translate(double x, double y) { cairo_translate(cr, x, y); }
  inline void scale(const base::Point &p) { cairo_scale(cr, p.x, p.y); }
  ...
  void get_text_extents(const FontSpec &font, const char *text,
                        cairo_text_extents_t &extents);
  bool get_font_extents(const FontSpec &font, 
  ...
  inline void move_to(const base::Point &pt) { cairo_move_to(cr, pt.x, pt.y); }
  inline void move_to(double x, double y) { cairo_move_to(cr, x, y); }
  inline void rel_move_to(double x, double y) { cairo_rel_move_to(cr, x, y); }

  inline void line_to(const base::Point &pt) { cairo_line_to(cr, pt.x, pt.y); }
  ...
  inline void rectangle(const base::Rect &rect) { cairo_rectangle(cr, rect.left(), rect.top(), rect.width(), rect.height()); }
