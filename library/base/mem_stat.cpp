...
void MemUsage::PrintUsage()
{
  ...
  logDebug("Total Physical Memory(RAM): %s Bytes, %s MB\n", std::to_string(totalPhysMem).c_str(), std::to_string(totalPhysMem / (1024 * 1024)).c_str());
  DWORDLONG physMemUsed = memInfo.ullTotalPhys - memInfo.ullAvailPhys;
  logDebug("Physical Memory currently used : %s Bytes, %s MB\n", std::to_string(physMemUsed).c_str(), std::to_string(physMemUsed / (1024 * 1024)).c_str());
  ...
  //logDebug("Total Physical Memory(RAM): %s Bytes, %s MB\n", base::to_string(totalPhysMem).c_str(), base::to_string(totalPhysMem / (1024 * 1024)).c_str());
  ...
  logDebug("Physical Memory currently used by current process : %s Bytes, %s MB\n", std::to_string(physMemUsedByMe).c_str(), std::to_string(physMemUsedByMe / (1024 * 1024)).c_str());
  logDebug("Difference : %s Bytes, %s MB\n", std::to_string(physMemUsedByMeAfter).c_str(), std::to_string(physMemUsedByMeAfter / (1024 * 1024)).c_str());
}
