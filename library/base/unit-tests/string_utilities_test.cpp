...
static bool compare_string_lists(const std::vector<std::string> &slist, const char *check[]) {
  size_t i;
...
TEST_FUNCTION(25) {
  ...
  ensure("Split by set 5", base::split_by_set(long_random_string, "\xA8").size() >
                             1); // Works only because our implementation is not utf-8 aware.
  ...
  ensure("Split by set 8",
         compare_string_lists(base::split_by_set("\"Lorem\"\t\"ipsum\"\t\"dolor\"\t\"sit\"\t\"amet\"", "\t"), input5));

  const char *input6[] = {"\"Lorem\"", "\"ipsum\"", "\"dolor\"", "\"sit\"", "\"amet\"", NULL};
  ensure(
    "Split by set 9",
    compare_string_lists(base::split_by_set("\"Lorem\"\t\"ipsum\"\n\"dolor\"\t\"sit\"\n\"amet\"", " \t\n"), input6));

  const char *input7[] = {"\"Lorem\"", "\"ip", "sum\"", "\"dolor\"", "\"sit\"", "\"amet\"", NULL};
  ensure(
    "Split by set 10",
    compare_string_lists(base::split_by_set("\"Lorem\"\t\"ip sum\"\n\"dolor\"\t\"sit\"\n\"amet\"", " \t\n"), input7));

  const char *input8[] = {"", "Lorem", "", " ", "ipsum", "", " ", "dolor", "", " ", "sit", "", " ", "amet", "", NULL};
  ensure("Split by set 11",
         compare_string_lists(base::split_by_set("\"Lorem\", \"ipsum\", \"dolor\", \"sit\", \"amet\"", ",\""), input8));
  ensure(
    "Split by set 12",
    compare_string_lists(base::split_by_set("\"Lorem\", \"ipsum\", \"dolor\", \"sit\", \"amet\"", ",\"", 100), input8));
  const char *input9[] = {"", "Lorem", "", " ", "ipsum", "", " ", "dolor\", \"sit\", \"amet\"", NULL};
  ensure(
    "Split by set 13",
    compare_string_lists(base::split_by_set("\"Lorem\", \"ipsum\", \"dolor\", \"sit\", \"amet\"", ",\"", 7), input9));
}
...
TEST_FUNCTION(30) {
  ...
  ensure_equals("Trim 6", base::trim("\xE3\x8A\xA8\xE3\x8A\xA8\x7F\xE3\x8A\xA8\xE3\x8A\xA8", long_random_string),
                "\x7F");
...
TEST_FUNCTION(35) {
  ...
  ensure("Unquoting 35.10", test_result[0] == 0 && test_result[1] == 10 && test_result[2] == 9 &&
                              test_result[3] == 13 && test_result[4] == '\b' && test_result[5] == 26);
...
TEST_FUNCTION(40) {
  ...
  ensure_equals("TEST 40.6: Starts with (starting on the second character)", base::hasPrefix(test_string, "his"),
                false);
  ensure_equals("TEST 40.7: Starts with (whole string)", base::hasPrefix(test_string, test_string), true);
  ensure_equals("TEST 40.8: Starts with (more then the original string)",
                base::hasPrefix(test_string, test_string + " Second part..."), false);
  ...
  ensure_equals("TEST 40.16: Ends with (starting on the second last character)", base::hasSuffix(test_string, "test"),
                false);
  ensure_equals("TEST 40.17: Ends with (whole string)", base::hasSuffix(test_string, test_string), true);
  ensure_equals("TEST 40.18: Ends with (more then the original string)",
                base::hasSuffix(test_string, test_string + " Second part..."), false);
  ...
  // Unicode tests
  ensure_equals("TEST 40.21: [Unicode]Starts with (success test)",
                base::hasPrefix(test_string_unicode, "\xC3\x89\xC3\x89"), true);
  ensure_equals("TEST 40.22: [Unicode]Starts with (exists somwehere in the middle)",
                base::hasPrefix(test_string_unicode, "is"), false);
  ensure_equals("TEST 40.23: [Unicode]Starts with (exists in the end)",
                base::hasPrefix(test_string_unicode, "\xE3\x8A\xA8"), false);
  ensure_equals("TEST 40.24: [Unicode]Starts with (search text don't exist)",
                base::hasPrefix(test_string_unicode, "blablabla"), false);
  ensure_equals("TEST 40.25: [Unicode]Starts with (search an empty string)", base::hasPrefix(test_string_unicode, ""),
                true);
  ensure_equals("TEST 40.26: [Unicode]Starts with (starting on the second character)",
                base::hasPrefix(test_string_unicode, "\x89\xC3\x89\xC3"), false);
  ensure_equals("TEST 40.27: [Unicode]Starts with (whole string)",
                base::hasPrefix(test_string_unicode, test_string_unicode), true);
  ensure_equals("TEST 40.28: [Unicode]Starts with (more then the original string)",
                base::hasPrefix(test_string_unicode, test_string_unicode + ". Second part..."), false);

  ensure_equals("TEST 40.29: [Unicode]Ends with (success test)", base::hasSuffix(test_string_unicode, ".\xE3\x8A\xA8"),
                true);
  ensure_equals("TEST 40.30: [Unicode]Ends with (exists somwehere in the middle)",
                base::hasSuffix(test_string_unicode, "to "), false);
  ensure_equals("TEST 40.31: [Unicode]Ends with (exists at the beginning)",
                base::hasSuffix(test_string_unicode, "\xC3\x89\xC3\x89"), false);
  ensure_equals("TEST 40.32: [Unicode]Ends with (search text don't exist)",
                base::hasSuffix(test_string_unicode, "blablabla"), false);
  ensure_equals("TEST 40.33: [Unicode]Ends with (search an empty string)", base::hasSuffix(test_string_unicode, ""),
                true);
  ensure_equals("TEST 40.34: [Unicode]Ends with (starting on the second last character)",
                base::hasSuffix(test_string_unicode, ".\xE3\x8A"), false);
  ensure_equals("TEST 40.35: [Unicode]Ends with (whole string)",
                base::hasSuffix(test_string_unicode, test_string_unicode), true);
  ensure_equals("TEST 40.36: [Unicode]Ends with (more then the original string)",
                base::hasSuffix(test_string_unicode, test_string_unicode + ". Second part..."), false);
}
...
TEST_FUNCTION(41) {
  ...
  ensure_equals("TEST 40.6: String left (more chars then the original string)", base::left(test_string, 50),
                test_string);
  ...
  ensure_equals("TEST 40.12: String right (more chars then the original string)", base::right(test_string, 50),
                test_string);

  //  Unicode tests
  ensure_equals("TEST 40.13: [Unicode]String left (regular testing)", base::left(test_string_unicode, 5),
                "\xC3\x89\xC3\x89\xC3");
  ...
  ensure_equals("TEST 40.17: [Unicode]String left (whole string)",
                base::left(test_string_unicode, test_string_unicode.length()), test_string_unicode);
  ensure_equals("TEST 40.18: [Unicode]String left (more chars then the original string)",
                base::left(test_string_unicode, 500), test_string_unicode);

  ensure_equals("TEST 40.19: [Unicode]String right (regular testing)", base::right(test_string_unicode, 5),
                "..\xE3\x8A\xA8");
  ...
  ensure_equals("TEST 40.23: [Unicode]String right (whole string)",
...
TEST_FUNCTION(43) {
  ...
  ensure_true(base::strfmt("TEST 43.1: Parsing for \"%s\"", font_description.c_str()),
              base::parse_font_description(font_description, font, size, bold, italic));
  ...
  ensure_true(base::strfmt("TEST 43.2: Parsing for \"%s\"", font_description.c_str()),
              base::parse_font_description(font_description, font, size, bold, italic));
  ensure_equals("", font, "Sans");
  ...
  ensure_true(base::strfmt("TEST 43.3: Parsing for \"%s\"", font_description.c_str()),
              base::parse_font_description(font_description, font, size, bold, italic));
  ensure_equals("", font, "Sans");
  ensure_equals("", size, 10);
  assure_true(bold);
  ...
  font_description = "Sans 10 BOLD";
  ensure_true(base::strfmt("TEST 43.4: Parsing for \"%s\"", font_description.c_str()),
              base::parse_font_description(font_description, font, size, bold, italic));
  ensure_equals("", font, "Sans");
  ensure_equals("", size, 10);
  assure_true(bold);
  ...
  ensure_true(base::strfmt("TEST 43.5: Parsing for \"%s\"", font_description.c_str()),
              base::parse_font_description(font_description, font, size, bold, italic));
  ensure_equals("", font, "Sans");
  ensure_equals("", size, 10);
  assure_false(bold);
  ...
  font_description = "Sans 10 ITALIC";
  ensure_true(base::strfmt("TEST 43.6: Parsing for \"%s\"", font_description.c_str()),
              base::parse_font_description(font_description, font, size, bold, italic));
  ensure_equals("", font, "Sans");
  ensure_equals("", size, 10);
  assure_false(bold);
  ...
  ensure_true(base::strfmt("TEST 43.7: Parsing for \"%s\"", font_description.c_str()),
              base::parse_font_description(font_description, font, size, bold, italic));
  ensure_equals("", font, "Sans");
  ensure_equals("", size, 10);
  assure_true(bold);
  ...
  font_description = "Sans 10 BOLD ITALIC";
  ensure_true(base::strfmt("TEST 43.8: Parsing for \"%s\"", font_description.c_str()),
              base::parse_font_description(font_description, font, size, bold, italic));
  ensure_equals("", font, "Sans");
  ensure_equals("", size, 10);
  assure_true(bold);
...
TEST_FUNCTION(50) {
  ...
  ensure_equals("Path normalization", normalize_path("/path///to/my//////dir"),
                base::replaceString("/path/to/my/dir", "/", separator));
  ensure_equals("Path normalization", normalize_path("D:\\files\\to//scan"),
                base::replaceString("D:/files/to/scan", "/", separator));
}
