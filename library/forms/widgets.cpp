...
static bool on_timer(int task_id, void* owner)
{
  ...
  for (vector<BaseWidget*>::const_iterator iterator= animated_widgets.begin(); iterator != animated_widgets.end(); iterator++)
    (*iterator)->step();
...
static void stop_animation_timer_for(BaseWidget* widget)
{
  ...
  for (vector<BaseWidget*>::iterator iterator= animated_widgets.begin(); iterator != animated_widgets.end(); iterator++)
    if (*iterator == widget)
    {
...
bool BaseWidget::compute_scale(double min, double max)
{
  ...
  for (ThresholdList::const_iterator iterator= _upper_thresholds.begin();
       iterator != _upper_thresholds.end(); iterator++)
    if (*iterator > max)
    {
      new_upper= *iterator;
      break;
    }
  for (ThresholdList::const_iterator iterator= _lower_thresholds.begin();
       iterator != _lower_thresholds.end(); iterator++)
    if (*iterator < min)
    {
      new_lower= *iterator;
      break;
...
{
  if (_background == NULL 
    || cairo_image_surface_get_width(_background) != _diagram_area.width()
    || cairo_image_surface_get_height(_background) != _diagram_area.height())
  {
    destroy_background();
    _background = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, (int)_diagram_area.width(), (int)_diagram_area.height());
...
: _value(0), _value_gradient(NULL), _grid(NULL)
{
...
void BarGraphWidget::prepare_background()
{
  ...
  if (_background == NULL 
    || cairo_image_surface_get_height(_background) != bounds.height())
  {
    destroy_background();
