  ...
  bool resizeChildren)
{
  ...
  for each (Control^ control in box->Controls)
  {
    ...
    {
      entry->bounds = System::Drawing::Rectangle(Point(0, 0), control->GetPreferredSize(Size(0, proposedSize.Height)));
      if (use_min_width)
  ...
  if (resizeChildren)
  {
    ...
    for each (ControlEntry^ entry in right_aligned)
      entry->bounds.Offset(proposedSize.Width, 0);
  ...
  bool resizeChildren)
{
  ...
  for each (Control^ control in box->Controls)
  {
    ...
    {
      entry->bounds= System::Drawing::Rectangle(Point(0, 0), control->GetPreferredSize(System::Drawing::Size(proposedSize.Width, 0)));
      if (use_min_width)
        entry->bounds.Width= control->MinimumSize.Width;
      if (use_min_height)
        entry->bounds.Height= control->MinimumSize.Height;
  ...
  if (resizeChildren)
  {
    ...
    for each (ControlEntry^ entry in bottom_aligned)
      entry->bounds.Offset(0, proposedSize.Height);
...
BoxWrapper::BoxWrapper(mforms::Box *box)
  : ViewWrapper(box)
{
}
