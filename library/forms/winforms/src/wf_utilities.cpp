...
void CustomMessageBox::ComputeLayout()
{
  ...
  if (buttonSize.Width > 0)
  {
    location = Drawing::Point(ClientSize.Width - buttonSize.Width, ClientSize.Height - buttonSize.Height - Padding.Bottom);
    if (_button1->Enabled)
    {
...
void UtilitiesWrapper::load_passwords()
{
  ...
  std::string user_info = _(std::string("If loading the passwords fails repeatedly you should clear the vault. This will remove all ") +
