...
namespace mforms {
  ...
  public:
    virtual int add_section(const std::string& name, const std::string &title, TaskSectionFlags flags = TaskSectionPlain) = 0;
    virtual void remove_section(const std::string& name) = 0;
    virtual int add_section_entry(const std::string& section_name, const std::string &entry_name, const std::string& title,
      const std::string& icon, TaskEntryType type) = 0;
  ...
  public: // Only for advanced sidebar
    ...
    virtual boost::signals2::signal<void (const std::string&)>* signal_filter_changed() { return NULL; }
    virtual boost::signals2::signal<void (void)>* tree_node_selected() { return NULL; }

    virtual mforms::ContextMenu *get_context_menu() { return NULL; }
