...
//  _darea.signal_expose_event().connect(sigc::bind(sigc::mem_fun(this, &DrawBoxImpl::repaint), self));
  ...
  _darea.signal_button_press_event().connect(sigc::bind(sigc::mem_fun(this, &DrawBoxImpl::mouse_button_event), self));
  _darea.signal_button_release_event().connect(sigc::bind(sigc::mem_fun(this, &DrawBoxImpl::mouse_button_event), self));
  ...
  _darea.signal_motion_notify_event().connect(sigc::bind(sigc::mem_fun(this, &DrawBoxImpl::mouse_move_event), self));
...
bool DrawBoxImpl::mouse_button_event(GdkEventButton *event, ::mforms::DrawBox *self)
{
  ...
  else if (event->type == GDK_BUTTON_RELEASE)
  {
    ...
    //We return false because otherwise when we do drag drop with Gtk::main::run wb will not be blocked and there will be problem.
    return false;
  }
