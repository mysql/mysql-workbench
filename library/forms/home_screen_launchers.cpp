...
LaunchersSection::LaunchersSection(mforms::HomeScreen *owner):
    HomeScreenSection("sidebar_launchers.png")
{
  _owner = owner;
  _context_menu = NULL;
  _action_menu = NULL;
...
{
  int width = get_width();
  if (x < LAUNCHERS_LEFT_PADDING || x > (width - LAUNCHERS_RIGHT_PADDING)
  ...
  y -= LAUNCHERS_TOP_PADDING;
  if ((y % (LAUNCHERS_ENTRY_HEIGHT + LAUNCHERS_VERTICAL_SPACING))
      > LAUNCHERS_ENTRY_HEIGHT)
  ...
  int row_bottom = row * (LAUNCHERS_ENTRY_HEIGHT + LAUNCHERS_VERTICAL_SPACING)
      + LAUNCHERS_ENTRY_HEIGHT;
...
void LaunchersSection::drawEntry(cairo_t *cr, const LauncherEntry &entry,
                                  bool hot)
{
  mforms::Utilities::paint_icon(cr, entry.icon, entry.bounds.left(),
      entry.bounds.top());
...
void LaunchersSection::layout(cairo_t *cr)
{
  if (is_layout_dirty())
  {
    ...
    _launcher_heading_rect = base::Rect(heading_left, LAUNCHERS_TOP_BASELINE,
    ...
    for (std::vector<LauncherEntry>::iterator iterator = _launchers.begin();
        iterator != _launchers.end(); iterator++)
    {
...
{
  int tilesPerRow = (get_width() - LAUNCHERS_LEFT_PADDING
      - LAUNCHERS_RIGHT_PADDING) / (LAUNCHERS_ENTRY_WIDTH + LAUNCHERS_SPACING);

  if (!_launchers.empty() && tilesPerRow > 1)
  {
    int height = (int)((_launchers.size() / tilesPerRow)
        * (LAUNCHERS_ENTRY_HEIGHT + LAUNCHERS_VERTICAL_SPACING)
...
void LaunchersSection::repaint(cairo_t *cr, int areax, int areay, int areaw,
                               int areah)
{
  int width = get_width();
  ...
  cairo_set_source_rgb(cr, 0, 0, 0);
  textWithDecoration(cr, _launcher_heading_rect.left(),
      _launcher_heading_rect.top(), _("Shortcuts"),
      false /*_hot_heading == ModelsOnly*/, _launcher_heading_rect.width());

//  cairo_set_operator(cr, CAIRO_OPERATOR_OVER);

  int row = 0;
  base::Rect bounds(0, LAUNCHERS_TOP_PADDING, LAUNCHERS_ENTRY_WIDTH,
      LAUNCHERS_ENTRY_HEIGHT);
  bool done = false;
  while (!done)
  {
...
void LaunchersSection::addLauncher(const std::string &icon, const std::string &name, const std::string &description, const base::any &obj)
{
  LauncherEntry entry;
  entry.title = name;
...
bool LaunchersSection::mouse_double_click(mforms::MouseButton button, int x,
                                          int y)
{
