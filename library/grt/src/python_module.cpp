...
static PyObject* function_call(PyGRTFunctionObject *self, PyObject *args, PyObject *kw)
{
  ...
  if ((int)self->function->arg_types.size() != PyTuple_Size(args))
  {
    PyErr_SetString(PyExc_TypeError, 
                    strfmt("%s.%s() takes %i arguments (%i given)", 
                           self->module->name().c_str(), self->function->name.c_str(),
                           (int) self->function->arg_types.size(), 
                           (int) PyTuple_Size(args)).c_str());
    return NULL;
  }
       ...
       arg != self->function->arg_types.end(); ++arg)
  {
    ...
    try
    {
      ...
      {
        PyErr_SetString(PyExc_TypeError, 
                        strfmt("%s.%s(): argument %i must be a %s but is None",
                               self->module->name().c_str(), self->function->name.c_str(),
                               (int) (a + 1), grt::fmt_type_spec(arg->type).c_str()).c_str());
        return NULL;
      }   
      grtargs.ginsert(v);
    }
    ...
    {
      PyErr_SetString(PyExc_TypeError, 
                      strfmt("%s.%s(): argument %i must be a %s", 
                             self->module->name().c_str(), self->function->name.c_str(),
                             (int) (a + 1), grt::fmt_type_spec(arg->type).c_str()).c_str());
      return NULL;
    }
...
{ 
  if (PyString_Check(attr_name)) 
  {
    ...
    {
      return Py_BuildValue("[ss]", "__doc__", "__bundlepath__", "__author__", "__name__", "__iconpath__", "__description__", "__version__");
    }
...
{(char*)"__doc__", (getter)module_get_doc, NULL, (char*)"Documentation of the GRT module.", 0},
{0, 0, 0, 0, 0},
};
