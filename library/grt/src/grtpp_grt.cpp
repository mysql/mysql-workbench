...
std::string Message::format(bool withtype) const
{
  ...
  if (withtype)
  {
    switch (type)
    {
    case InfoMsg:  text= "Info: "; break;
    case WarningMsg: text= "Warning: "; break;
    case ErrorMsg: text= "Error: "; break;
    default: text= ""; break;
    }
  }
...
: std::logic_error(std::string("Type mismatch: expected type ").append(type_to_str(expected)).append(", but got ").append(type_to_str(actual)))
{
}

type_error::type_error(TypeSpec expected, TypeSpec actual)
: std::logic_error(std::string("Type mismatch: expected ").append(fmt_type_spec(expected)).append(", but got ").append(fmt_type_spec(actual)))
{
  
}

type_error::type_error(Type expected, Type actual, Type container)
: std::logic_error(std::string("Type mismatch: expected content-type ").append(type_to_str(expected)).append(", but got ").append(type_to_str(actual)))
{
}

type_error::type_error(const std::string &expected, Type actual)
: std::logic_error(std::string("Type mismatch: expected ").append(expected).append(", but got ").append(type_to_str(actual)))
{
}

type_error::type_error(const std::string &expected, const std::string &actual)
: std::logic_error(std::string("Type mismatch: expected object of type ").append(expected).append(", but got ").append(actual))
{
}

type_error::type_error(const std::string &expected, const std::string &actual, Type container)
: std::logic_error(std::string("Type mismatch: expected content object of type ").append(expected).append(", but got ").append(actual))
{
}  
...
GRT::GRT()
: _check_serialized_crc(false), _verbose(false)
{
...
GRT::~GRT()
{
  ...
  */
  ...
  for (std::map<std::string,MetaClass*>::iterator iter= _metaclasses.begin();
       iter != _metaclasses.end(); ++iter)
    delete iter->second;
  _metaclasses.clear();
}
  ...
  if (check_class_binding)
  {
         ...
         iter != _metaclasses.end(); ++iter)
    {
      if (!iter->second->is_bound())
        g_warning("Allocation function of '%s' is unbound, which probably means the implementing C++ class was not registered\n",
...
{
  for (std::list<ModuleLoader*>::const_iterator iter= _loaders.begin();
       iter != _loaders.end(); ++iter)
  {
                         ...
                         bool reload)
{
  ...
  while ((entry= g_dir_read_name(dir)) != NULL)
  {
    ...
    if (!exts.empty())
    {
      ...
      for (std::list<std::string>::const_iterator ext= exts.begin();
        ext != exts.end(); ++ext)
      {
        std::string e= *ext;
...
grt::ValueRef GRT::call_module_function(const std::string &module, const std::string &function, const grt::BaseListRef &args)
{
...
std::vector<Module*> GRT::find_modules_matching(const std::string &interface_name, const std::string &name_pattern)
{
  ...
  for (std::vector<Module*>::const_iterator module= _modules.begin(); 
       module != _modules.end(); ++module)
  {
    ...
    if (!interface_name.empty())
    {
      ...
      if (std::find((*module)->get_interfaces().begin(), (*module)->get_interfaces().end(), 
                    interface_name) != (*module)->get_interfaces().end())
        ok= true;
    }

    if (ok && (name_pattern.empty() || g_pattern_match_simple(name_pattern.c_str(),
                                                              (*module)->name().c_str())))
    {
