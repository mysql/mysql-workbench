...
TEST_FUNCTION(3)
{
  ...
  ensure_equals("type_error expected/actual strings c-tor", error->what(), "Type mismatch: expected object of type foo,"
    " but got bar");
  delete error;

  error= new type_error("foo", "bar", container);
  ensure_equals("type_error expected/actual strings in container c-tor", error->what(), "Type mismatch: expected "
    "content object of type foo, but got bar");
  delete error;

  error= new type_error(expected, actual);
  ensure_equals("type_error expected/actual GRT values c-tor", error->what(), "Type mismatch: expected "
    "type string, but got real");
  delete error;

  error= new type_error(expected, actual, container);
  ensure_equals("type_error expected/actual GRT values in container c-tor", error->what(), "Type mismatch: expected "
