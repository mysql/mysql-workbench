...
class TestModuleImpl : public ModuleImplBase
...
public:
  ...
  DEFINE_INIT_MODULE("1.0", "", ModuleImplBase,
                    DECLARE_MODULE_FUNCTION(TestModuleImpl::returnNull),
                     NULL
                    );

  ObjectRef returnNull() { return ObjectRef(); }
};
...
TEST_FUNCTION(1)
{
  _grt.load_metaclasses("data/structs.test.xml");
  _grt.end_loading_metaclasses();
  
  // this is exactly what should be done
  // by module dll during initialization
  InterfaceImplBase::Register<SampleInterface1Impl>(&_grt);
  InterfaceImplBase::Register<SampleInterface2Impl>(&_grt);

  _grt.get_native_module<SampleModule1Impl>();
  _grt.get_native_module<SampleModule2Impl>();
  _grt.get_native_module<SampleModule3Impl>();

  _grt.get_native_module<TestModuleImpl>();

  try 
  {
    _grt.get_native_module<BadModuleImpl>();
...
TEST_FUNCTION(4)
{
  ensure_equals("number of interfaces", _grt.get_interfaces().size(), 2U);
  ensure_equals("number of modules", _grt.get_modules().size(), 4U);

  // interfaces[0]
  const Interface *iface= _grt.get_interface("SampleInterface1");
  ...
  // interfaces[1]
  iface= _grt.get_interface("SampleInterface2");
...
TEST_FUNCTION(5)
{
  ...
  ensure("modules[0] implements", _grt.get_interface(m->get_interfaces()[0])!=0);
  ...
  ensure("modules[1] implements", _grt.get_interface(m->get_interfaces()[0])!=0);
  ...
  ensure_equals("modules[2] functions[3] params[1] object_class", f->arg_types[1].type.base.object_class, "test.Author"); // unsure
