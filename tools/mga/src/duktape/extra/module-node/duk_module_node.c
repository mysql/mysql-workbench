...
static duk_ret_t duk__handle_require(duk_context *ctx) {
	...
	if (duk_is_object(ctx, -1)) {
		// Probably loaded from JSON file. Let's not cache this for now.
    duk__del_cached_module(ctx, id);
		return 1;
	} else if (duk_is_string(ctx, -1)) {
		duk_int_t localResult;

		/* [ ... module source ] */

#if DUK_VERSION >= 19999
		localResult = duk_safe_call(ctx, duk__eval_module_source, NULL, 2, 1);
#else
		localResult = duk_safe_call(ctx, duk__eval_module_source, 2, 1);
#endif
		if (localResult != DUK_EXEC_SUCCESS) {
			duk__del_cached_module(ctx, id);
			duk_throw(ctx);  /* rethrow */
		}
