...
namespace HtmlRenderer.Utils
{
    ...
    internal sealed class DomUtils
    {
        ...
        /// <summary>
        /// Get css link box under the given sub-tree with the given id.<br/>
        /// </summary>
        /// <param name="box">the box to start search from</param>
        /// <param name="id">the id to find the box by</param>
        /// <returns>css link box if exists or null</returns>
        public static CssBox GetLinkBox(CssBox box, string id)
        {
            if( box != null && !string.IsNullOrEmpty(id) )
            {
                if( box.HtmlTag != null && box.HtmlTag.Name.Equals(HtmlConstants.A, StringComparison.OrdinalIgnoreCase) && id.Equals(box.HtmlTag.TryGetAttribute("id"), StringComparison.OrdinalIgnoreCase) )
                {
                    return box;
                }
                ...
                foreach(var childBox in box.Boxes)
                {
                    var foundBox = GetLinkBox(childBox, id);
                    if( foundBox != null )
                        return foundBox;
                }
            }
