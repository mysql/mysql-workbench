...
Recordset_sqlite_storage::Recordset_sqlite_storage(GRTManager *grtm)
:
...
void Recordset_sqlite_storage::do_fetch_blob_value(Recordset *recordset, sqlite::connection *data_swap_db, RowId rowid, ColumnId column, sqlite::variant_t &blob_value)
{
  Recordset::Column_names &column_names= get_column_names(recordset);
...
//unused  Recordset::Column_types &real_column_types= get_real_column_types(recordset);
  ...
  std::string sql_query= decorated_sql_query(column_names);
  {
    ...
    sql_query= strfmt("select `%s` from (%s) t where %s",
  ...
  if (rs_contains_rows)
  {
    do (blob_value = rs->get_variant(0));
    while (rs->next_row());
  }
}
