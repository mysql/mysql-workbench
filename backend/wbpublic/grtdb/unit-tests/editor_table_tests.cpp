...
{
public:
TestTableColumnsListBE(TableEditorBE *ed)
 : TableColumnsListBE(ed) {}

virtual std::vector<std::string> get_datatype_names() { return  std::vector<std::string>(); } 
};
...
TEST_FUNCTION(1)
{
  ...
  db_mysql_SchemaRef schema= db_mysql_SchemaRef();
...
//  ensure("set name type", flag);
  ...
  flag= clist->get_row(0,
                       name,
                       type,
                       ispkb,
                       notnullb, isunique, isbin, isunsigned, iszerofill,
                       flags,
                       defvalue,
                       charset,
                       collation,
...
TEST_FUNCTION(11)
{ // delete a column and see index deletion
  ensure("table editor !NULL", editor != NULL);
...
TEST_FUNCTION(12)
{
  ...
  db_TableRef table = db_mysql_TableRef();
