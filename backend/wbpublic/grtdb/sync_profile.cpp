...
db_mgmt_SyncProfileRef bec::create_sync_profile(workbench_physical_ModelRef model, const std::string &profile_name, const std::string &target_schema)
{
  ...
  model->syncProfiles().set(base::strfmt("%s::%s", profile->targetHostIdentifier().c_str(), profile->targetSchemaName().c_str()), profile);

  return profile;
}

db_mgmt_SyncProfileRef bec::get_sync_profile(workbench_physical_ModelRef model, const std::string &profile_name, const std::string &target_schema)
{
  return db_mgmt_SyncProfileRef::cast_from(model->syncProfiles().get(base::strfmt("%s::%s", profile_name.c_str(), target_schema.c_str())));
}
...
void bec::update_schema_from_sync_profile(db_SchemaRef schema, db_mgmt_SyncProfileRef profile)
{
  ...
  GRTLIST_FOREACH(db_View, schema->views(), view)
  {
    ...
    (*view)->oldModelSqlDefinition(grt::StringRef::cast_from(profile->lastKnownViewDefinitions().get((*view).id()+":model", (*view)->oldModelSqlDefinition())));
    (*view)->oldServerSqlDefinition(grt::StringRef::cast_from(profile->lastKnownViewDefinitions().get((*view).id()+":server", (*view)->oldServerSqlDefinition())));
  }
