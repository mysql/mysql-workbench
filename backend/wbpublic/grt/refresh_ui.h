...
class WBPUBLICBACKEND_PUBLIC_FUNC RefreshUI
{
  public:
    ...
    struct Blocker
    {
      Blocker(RefreshUI& o) : _obj(&o) {_obj->block_auto_refresh();}
      ~Blocker() {_obj->unblock_auto_refresh();}

      RefreshUI     *_obj;
    };

    void block_auto_refresh()    { _partial_refresh_blocked = true; }
    void unblock_auto_refresh()  { _partial_refresh_blocked = false; }
  private:
