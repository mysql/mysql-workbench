...
void WBContext::init_grt_tree(WBOptions *options, std::shared_ptr<grt::internal::Unserializer> unserializer)
{
  ...
  grouplist= ListRef<db_DatatypeGroup>::cast_from(_grt->unserialize(base::makePath(options->basedir, TYPE_GROUP_FILE), unserializer));
...
{
  return grt::ListRef<app_PaperType>::cast_from(_grt->unserialize(base::makePath(get_datadir(),
  ...
  if (g_file_test(inst_list_xml.c_str(), G_FILE_TEST_EXISTS))
  {
    try
    {
      grt::ListRef<db_mgmt_ServerInstance> 
        list(grt::ListRef<db_mgmt_ServerInstance>::cast_from(_grt->unserialize(inst_list_xml)));
...
void WBContext::load_other_connections()
{
  ...
  if (g_file_test(conn_list_xml.c_str(), G_FILE_TEST_EXISTS))
  {
    try
    {
      grt::ListRef<db_mgmt_Connection> list(grt::ListRef<db_mgmt_Connection>::cast_from(_grt->unserialize(conn_list_xml)));
...
void WBContext::save_app_options()
{
  ...
  _grt->serialize(options, options_file + ".tmp", OPTIONS_DOCUMENT_FORMAT,
...
void WBContext::save_connections()
{
  ...
  _grt->serialize(mgmt->storedConns(),
 ...
 */
void WBContext::load_starters(std::shared_ptr<grt::internal::Unserializer> unserializer)
{
  ...
  if (g_file_test(starters_file.c_str(), G_FILE_TEST_EXISTS))
  {
    ...
    try 
    {
      xmlDocument= _grt->load_xml(starters_file);
      base::ScopeExitTrigger free_on_leave(boost::bind(xmlFreeDoc, xmlDocument));
      ...
      grt::ListRef<app_Starter> new_starters=
        grt::ListRef<app_Starter>::cast_from(_grt->unserialize_xml(xmlDocument, starters_file));
    ...
    catch (std::exception &exc)
    {
      ...
      _grt->send_warning(strfmt("Error while loading '%s': %s",
  ...
  if (g_file_test(starters_file.c_str(), G_FILE_TEST_EXISTS))
  {
    ...
    try 
    {
      ...
      starter_links= grt::ListRef<app_Starter>::cast_from(_grt->unserialize_xml(xmlDocument, starters_file));
    }
    catch (std::exception &exc)
    {
      mforms::Utilities::show_error(_("Error while loading starters"),
...
void WBContext::save_app_state()
{
  ...
  _grt->serialize(get_root()->state(), state_file + ".tmp", STATE_DOCUMENT_FORMAT,
...
/*  catch (grt::grt_runtime_exception &exc)
  ...
  if (!warnings.empty())
  {
    ...
    _grt->send_output(base::strfmt("%i problems found and corrected during load of file '%s':\n", (int)warnings.size(), file.c_str()));
    for (std::list<std::string>::const_iterator iter= warnings.begin(); iter != warnings.end(); ++iter)
...
bool WBContext::install_module_file(const std::string &path)
{
  ...
  else if (!copy_file(path.c_str(), target_path.c_str()))
  {
   ...
   _grt->send_output(strfmt("ERROR: could not copy module '%s' to '%s': %s\n",
