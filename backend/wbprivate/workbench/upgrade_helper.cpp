...
void XMLTraverser::cache_object_nodes(xmlNodePtr node)
{
  if (node == NULL)
  {
    log_error("XMLTraverser::cache_object_nodes node is NULL");
    return;
  }
...
std::vector<xmlNodePtr> XMLTraverser::scan_objects_of_type(const char *struct_name)
{
  ...
  for (std::map<std::string,xmlNodePtr>::iterator iter= nodes_by_id.begin(); iter != nodes_by_id.end();
    ++iter)
  {
...
void find_replace_xml_attributes(xmlNodePtr root, const char **attr_list, const char **from_list, const char **to_list)
{
    ...
    if (child->type == XML_ELEMENT_NODE)
    {
      if (klass_name)
      {
        ...
        if (key)
        {
          ...
          {
            if (strcmp(klass[i], (char*)klass_name) == 0 &&
                strcmp(name_from[i], (char*)key) == 0)
            {
