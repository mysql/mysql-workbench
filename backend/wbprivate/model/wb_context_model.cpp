...
void WBContextModel::free_canvas_view(mdc::CanvasView *view)
{
  ...
  if (diagram != NULL)
  {
    ...
    if (bec::GRTManager::get()->in_main_thread())
      ...
      std::bind(wb::WBContextUI::get()->get_wb()->_frontendCallbacks.destroy_view, view));
  }
}
...
void WBContextModel::export_png(const std::string &path)
{
  ...
  if (form)
  {
    wb::WBContextUI::get()->get_wb()->_frontendCallbacks.show_status_text(strfmt(_("Exporting to %s..."), path.c_str()));
    try
    {
      form->get_view()->export_png(path, true);
      wb::WBContextUI::get()->get_wb()->_frontendCallbacks.show_status_text(strfmt(_("Exported diagram image to %s"), path.c_str()));
    }
...
void WBContextModel::export_pdf(const std::string &path)
{
  ...
  if (form)
  {
    ...
    wb::WBContextUI::get()->get_wb()->_frontendCallbacks.show_status_text(strfmt(_("Exporting full model diagram to %s..."), path.c_str()));
    
    try
    {
      form->get_view()->export_pdf(path, size);
      wb::WBContextUI::get()->get_wb()->_frontendCallbacks.show_status_text(strfmt(_("Exported PDF to %s"), path.c_str()));
    }
    catch (const std::exception &exc)
    {
...
void WBContextModel::export_svg(const std::string &path)
{
  ...
  if (form)
  {
    ...
    try
    {
      ...
      wb::WBContextUI::get()->get_wb()->_frontendCallbacks.show_status_text(strfmt(_("Exported SVG to %s"), path.c_str()));
    }
    catch (const std::exception &exc)
    {
...
void WBContextModel::export_ps(const std::string &path)
{
  ...
  if (form)
  {
    ...
    try
    {
      ...
      wb::WBContextUI::get()->get_wb()->_frontendCallbacks.show_status_text(strfmt(_("Exported PS to %s"), path.c_str()));
    }
    catch (std::exception &exc)
    {
