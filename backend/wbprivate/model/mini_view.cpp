  ...
  if (btn == mdc::ButtonLeft && _viewport_figure)
  {
    ...
    else
    {
      if (_click_pos == pos)
      {
        ...
        if (nrect.left() < bounds.left()) nrect.pos.x= bounds.left();
        if (nrect.top() < bounds.top()) nrect.pos.y= bounds.top();
        if (nrect.right() > bounds.right()) nrect.pos.x= bounds.right()-nrect.width();
        if (nrect.bottom() > bounds.bottom()) nrect.pos.y= bounds.bottom()-nrect.height();
