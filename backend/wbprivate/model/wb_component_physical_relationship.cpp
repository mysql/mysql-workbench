...
: owner(owner), view(form), state(RPickingStart), type(rtype), floater(0)
{
  ...
  owner->get_wb()->show_status_text(last_message);
}
...
void WBComponentPhysical::RelationshipToolContext::cancel()
{
  ...
  if (state != RFinished)
    owner->get_wb()->show_status_text(_("Cancelled."));
}
...
bool WBComponentPhysical::RelationshipToolContext::pick_table(const workbench_physical_TableFigureRef &table)
{
  ...
  {
    last_message= strfmt(_("'%s' has no Primary Key. Please add a PK or select another Table."),
  ...
  if (type == RelationshipnmId)
    last_message= strfmt(_("'%s' selected. Please select the second Table."),
                       table->table()->name().c_str());
  else
    last_message= strfmt(_("'%s' selected. Please select the Referenced Table."),
                    table->table()->name().c_str());
...
bool WBComponentPhysical::RelationshipToolContext::pick_column(const workbench_physical_TableFigureRef &table, const db_ColumnRef &column)
{
  if (column.is_valid())
  {
  ...
  else // !column.is_valid()
  {
    if (!columns.empty())
    {
      ...
      last_message= strfmt(_("Pick other columns that belong to the Foreign Key or proceed to pick the referenced table or matching columns in it."));
      return false;
    }
    else
...
bool WBComponentPhysical::RelationshipToolContext::pick_refcolumn(const workbench_physical_TableFigureRef &table, const db_ColumnRef &column)
{
  if (column.is_valid())
  {
    // if so, then make sure that all columns belong to the same table and
...
bool WBComponentPhysical::RelationshipToolContext::finish_for_columns()
{
  ...
  switch (type)
  {
    case Relationship11Id:    imany= false; fmany= false; /*identifying= true;*/ break;
    case Relationship1nId:    imany= true;  fmany= false; /*identifying= true;*/ break;
    case Relationship11NonId: imany= false; fmany= false; /*identifying= false;*/ break;
    case Relationship1nNonId: imany= true;  fmany= false; /*identifying= false;*/ break;
    case RelationshipnmId:    imany= true;  fmany= true;  break;
    case RelationshipPick:    imany= true;  fmany= false; /*identifying= true;*/ break;
    default: break;
  }
  ...
  if (imany && fmany)
  {
    ...
    if (ok)
    {
      ...
      last_message= strfmt(_("Relationship between '%s' and '%s' created through an associative table."),
                        itable->table()->name().c_str(),
                        ftable->table()->name().c_str());
    }
    else
      last_message= strfmt(_("Cannot create relationship between '%s' and '%s'."), 
                        itable->table()->name().c_str(),
                        ftable->table()->name().c_str());
  }
  else
  {
    ...
    fk= bec::TableHelper::create_foreign_key_to_table(itable->table(), columns,
                                                      ...
                                                      owner->get_wb()->get_wb_options(),
                                                      view->get_model_diagram()->owner()->options());
    if (fk.is_valid())
    {
      undo.end(_("Create Relationship"));
      
      last_message= strfmt(_("Relationship between '%s' and '%s' created."), 
                        itable->table()->name().c_str(), 
                        ftable->table()->name().c_str());
    }
    else
...
bool WBComponentPhysical::RelationshipToolContext::finish_for_tables()
{
  ...
  switch (type)
  {
    ...
    case Relationship1nNonId: imany= true;  fmany= false; identifying= false; break;
    case RelationshipnmId:    imany= true;  fmany= true;  break;
    case RelationshipPick:    imany= true;  fmany= false; identifying= false; break;
    default: break;
  }
  imand= view->get_tool_argument("workbench.physical.Connection:optional")!="1";
  fmand= view->get_tool_argument("workbench.physical.Connection:refOptional")!="1";  
...
void WBComponentPhysical::RelationshipToolContext::on_figure_crossed(const model_ObjectRef &owner, mdc::CanvasItem *item, bool enter, const Point &pos)
{
