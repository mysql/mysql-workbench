                                 ...
                                 workbench_physical_ModelRef &model)
{
  ...
  // set version
  GrtVersionRef version = parse_version(grt, db_version);
...
db_SchemaRef WBComponentPhysical::add_new_db_schema(const workbench_physical_ModelRef &model)
{
  ...
  schema= get_grt()->create_object<db_Schema>(class_name);
...
grt::DictRef WBComponentPhysical::delete_db_schema(const db_SchemaRef &schema,
                                                   bool check_empty)
{
  if (check_empty && 
...
void WBComponentPhysical::delete_db_schema(const db_SchemaRef &schema)
{
  ...
  if (info.is_valid())
  {
    ...
    if (info.get_int("tables") > 0)
      objects+= strfmt("%zi tables, ", info.get_int("tables"));
    if (info.get_int("views") > 0)
      objects+= strfmt("%zi views, ", info.get_int("views"));
    if (info.get_int("routines") > 0)
      objects+= strfmt("%zi routines, ", info.get_int("routines"));
...
model_FigureRef WBComponentPhysical::place_db_object(ModelDiagramForm *view,
                                                       const Point &pos,
      ...
      const std::list<db_DatabaseObjectRef> &objects, grt::CopyContext& copy_context)
{
  ...
  if (objects.empty())
  {
    mforms::Utilities::show_message(_("Cannot Place Object"), _("The object cannot be placed in the diagram."), _("Close"));
  ...
  for (std::list<db_DatabaseObjectRef>::const_iterator iter= objects.begin(); iter != objects.end(); ++iter)
  {
    ...
    {
      int r = mforms::Utilities::show_message(_("Place Object in Diagram"),
                                      base::strfmt(_("'%s' is already in this diagram. Would you like to duplicate the schema object and place a copy?"), obj->name().c_str()),
...
void WBComponentPhysical::place_new_db_object(ModelDiagramForm *vform, const Point &pos,
                                              wb::ObjectType type)
{
...
bool WBComponentPhysical::create_nm_relationship(ModelDiagramForm *view,
...
void WBComponentPhysical::copy_object_to_clipboard(const grt::ObjectRef &object,
