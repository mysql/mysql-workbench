...
namespace wb {
  ...
  class PhysicalModelDiagramFeatures : public base::trackable
  {
    ...
    void on_figure_double_click(const model_ObjectRef &owner, mdc::CanvasItem *item, const base::Point &pos, mdc::MouseButton button, mdc::EventState state);
    void on_figure_mouse_button(const model_ObjectRef &owner, mdc::CanvasItem *item, bool press, const base::Point &pos, mdc::MouseButton button, mdc::EventState state);
    ...
    bec::GRTManager::Timer *run_every(const boost::function<bool ()> &slot, double seconds);
