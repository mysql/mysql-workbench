...
public:
  ...
  void sql_editor_reorder(MySQL::Forms::AppViewDockContent ^page, Int32 to) { (*_ref)->sql_editor_reordered(dynamic_cast<SqlEditorPanel*>(page->GetBackend()), to); }
  String^ sql_editor_path(Int32 index) { if ((*_ref)->sql_editor_panel(index)) return CppStringToNative((*_ref)->sql_editor_panel(index)->filename()); else return ""; }
  
  Int32 exec_sql_error_count() { return (*_ref)->exec_sql_error_count(); };
  void show_output_area();

  void handle_tab_menu_action(String ^action, int tab) { (*_ref)->handle_tab_menu_action(NativeToCppString(action), tab); }

  void handle_history_action(String ^action, String ^sql) { (*_ref)->handle_history_action(NativeToCppString(action), NativeToCppString(sql)); }
