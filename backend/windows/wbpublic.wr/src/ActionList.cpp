...
ActionList::~ActionList()
{
  ...
  for each (KeyValuePair<String^, RowsColActionCallback^> ^pair in _rows_col_actions)
    delete pair->Value;
  delete _rows_col_actions;

  for each (KeyValuePair<String^, NodesActionCallback^> ^pair in _nodes_actions)
    delete pair->Value;
  delete _nodes_actions;

  for each (KeyValuePair<String^, NodeActionCallback^> ^pair in _node_actions)
    delete pair->Value;
  delete _node_actions;

  for each (KeyValuePair<String^, ActionCallback^> ^pair in _actions)
    delete pair->Value;
  delete _actions;
