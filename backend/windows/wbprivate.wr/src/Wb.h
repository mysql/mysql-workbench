...
public:
  ...
  {
    bool get() { return !inner->verbose; }
  }

  property String^ OpenAtStartup
  {
    String^ get();
  }

  property String^ OpenAtStartupType
  {
    String^ get();
  }
};
...
public:
  ...
  String^ get_description_for_selection(MySQL::Base::UIForm ^form, [Out] GrtValue^ %activeObjList, [Out] List<String^>^ %items);
