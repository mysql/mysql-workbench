...
DELIMITER ;;
CREATE TRIGGER `ins_film` AFTER INSERT ON `sakila`.`film` FOR EACH ROW BEGIN
...
CREATE TRIGGER `del_film` AFTER DELETE ON `sakila`.`film` FOR EACH ROW BEGIN
...
FROM sakila.customer AS cu JOIN address AS a ON cu.address_id = a.address_id JOIN city ON a.city_id = city.city_id 
...
FROM sakila.staff AS s JOIN address AS a ON s.address_id = a.address_id JOIN city ON a.city_id = city.city_id 
	JOIN country ON city.country_id = country.country_id;
