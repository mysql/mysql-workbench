...
{
  // make sure changes to a treeview are commited
...
void PluginEditorBase::embed_code_editor(mforms::View *container, Gtk::VBox *vbox, bool commit_on_focus_out)
{
  ...
  if (container)
  {
    ...
    if (commit_on_focus_out)
      editorw->signal_focus_out_event().connect(sigc::bind_return(sigc::hide(sigc::mem_fun(get_be(), &bec::BaseEditor::commit_changes)), false));
  }
}
