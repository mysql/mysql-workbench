...
class ActionAreaNotebookDockingPoint : public mforms::DockingPointDelegate
{
  ActionAreaNotebook *_notebook;
  std::string _type;

  void close_page(mforms::AppView *page);
public:
  ActionAreaNotebookDockingPoint(ActionAreaNotebook *note, const std::string &type)
  : _notebook(note), _type(type) {}

  void set_notebook(ActionAreaNotebook *note);
