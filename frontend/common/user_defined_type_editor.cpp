...
void UserDefinedTypeEditor::selected_row()
{
  ...
  if (!node)
  {
    for (std::vector<mforms::CheckBox*>::iterator iter= _flags.begin();
  ...
  else
  {
    ...
    int i= 0;
    for (grt::StringListRef::const_iterator iter= simpleType->flags().begin();
         iter != simpleType->flags().end(); ++iter, ++i)
    {
...
void UserDefinedTypeEditor::type_changed()
{
  if (_type.get_selected_index() >= 0)
  {
    ...
    for (grt::StringListRef::const_iterator iter= simpleType->flags().begin();
         iter != simpleType->flags().end(); ++iter)
    {
...
void UserDefinedTypeEditor::delete_clicked()
{
  ...
  if (node)
  {
    ...
    {
      mforms::Utilities::show_error(_("Delete User Type"),
        strfmt(_("Type '%s' is used in a column and cannot be deleted."), node->get_string(0).c_str()),
        _("OK"));
  ...
  for (int c= _type_list.count(), i= 0; i < c; i++)
  {
    ...
    if (names.find(name) != names.end())
    {
      mforms::Utilities::show_error(_("Duplicate Type Name"),
                                    strfmt(_("There are two or more types with the same name '%s'."),
                                           name.c_str()), _("OK"));
      return;
    }
    names.insert(name);
